...
exports.create = (req, res) => {
    ...
    form.parse(req, (err, fields, files) => {
        ...
        blog.title = title;
        // blog.body = body;
        blog.vehicleType = vehicleType;
        blog.isAc = isAc;
        blog.morningStartTime = morningStartTime;
        blog.morningEndTime = morningEndTime;
...
exports.list = (req, res) => {
    Blog.find({})
        ...
        .select('_id title slug categories tags postedBy createdAt updatedAt route vehicleType telephone')
        .exec((err, data) => {
            if (err) {
                return res.json({
                    error: errorHandler(err)
...
exports.listAllBlogsCategoriesTags = (req, res) => {
    ...
    Blog.find({})
        ...
        .skip(skip)
        .limit(limit)
        .select('_id title slug excerpt categories tags postedBy createdAt updatedAt route vehicleType telephone')
        .exec((err, data) => {
            if (err) {
                return res.json({
                    error: errorHandler(err)
...
exports.listRelated = (req, res) => {
    ...
    Blog.find({ _id: { $ne: _id }, categories: { $in: categories } })
        .limit(limit)
        .populate('postedBy', '_id name username profile')
        .select('title slug excerpt postedBy createdAt updatedAt route vehicleType telephone')
        .exec((err, blogs) => {
            if (err) {
                return res.status(400).json({
